...
- **`Screen`** `{ scroll=true, keyboard, refreshing, onRefresh, tabBar=true, edges=["top"], contentStyle }` — safe areas, 16 pt gutters, `cardGap` (12) between children, pull-to-refresh, bottom clearance for the tab bar. `keyboard` swaps in `KeyboardAwareScrollView`. Do: one `Screen` per route. Don't: nest `ScrollView`s; for FlashList use `scroll={false}` + `List` and pad with `useTabBarSpace()`.
  - **Top bar (tab screens, `tabBar` true).** A flat bar (page colour + hairline, `insets.top + SCREEN_TOP_BAR` tall) fades in over the first 16 pt of scroll, tied to the scroll position, so rows never slide visibly under the corner Floo or the status bar. Scrolling screens get it from their own scroller; **list screens (`scroll={false}`) get it through `List`**, which attaches `useScreenScroll()` to vertical lists by itself (and drops the bar again when it unmounts). Another scroller inside a list screen calls `useScreenScroll()?.(e)` from its `onScroll` (the hook drops its offset on unmount too) and wraps its content in `<InListContext.Provider value>` if a large `Header` scrolls inside it. Horizontal lists never drive it; modal screens (`tabBar={false}`) have none. If the large `Header` stays put *above* the list (not in its `ListHeaderComponent`), rows start below it and never reach Floo, so the bar stays off rather than hiding the title. The bar is drawn inside the screen and Floo one layer up (the tabs layout), so **nothing a screen draws can cover Floo** — don't add your own sticky header on top; if a screen needs one, give it the bar's height and colour.
