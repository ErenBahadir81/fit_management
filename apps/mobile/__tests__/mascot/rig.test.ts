...
import { COMPILED, COMPILED_MIRROR, GESTURES, GESTURE_INDEX, IDLE_GESTURES, MOOD_RIG } from "../../src/mascot/model/poses";
...
describe("pose library", () => {
  ...
  test("gestures start and end on the base pose (no snap back)", () => {
    for (const g of GESTURES) {
      ...
      c.channels.forEach((ch) => expect(at0[ch]).toBeCloseTo(base[ch], 5));
      const end = base.slice();
      const alive = applyGesture(c, c.duration - 1, end);
      expect(alive).toBe(true);
      c.channels.forEach((ch) => expect(Math.abs(end[ch] - base[ch])).toBeLessThan(Math.max(1, Math.abs(base[ch]) * 0.05) + 3));
